...
x_test = torch.empty(6, dtype=dtype)
...
if experiment['env_seed'] is None:
    ...
    env = AcrobotPrediction(generator=np.random.RandomState(experiment['env_seed']))
...
def test_pairwise_interference():
    ...
    for i in range(len(interference_x_test)):
        with torch.no_grad():
            observation = interference_x_test[i]
            return_ = interference_y_test[i]
            next_observation = env.get_next_observation(observation)
            next_return = AcrobotPrediction.get_return(next_observation)
            if experiment['loss'] == 'squared_error':
                y_test[0] = return_
            else:
                assert(experiment['loss'] == 'TD')
                for j in range(len(next_observation)):
                    x_test[j] = next_observation[j]
                y_test[0] = next_return - return_ + model(x_test)
            for j in range(len(observation)):
                x_test[j] = observation[j]
        y_pred = model(x_test)
