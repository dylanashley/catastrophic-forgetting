...
misc_labels = ["end_time", "outfile", "start_time"]
repeat_labels = [
    "init_seed",
    "shuffle_seed",
    "test_folds",
...
def get_summary(results):
    ...
    for _, row in results.iterrows():
        ...
        if key not in rv:
            ...
            if len(errors) > len(rv[key]["count"]):
                ...
                np.copyto(
                    new_second[: len(rv[key]["errors_second"])],
                    rv[key]["errors_second"],
                )
            ...
            rv[key]["total_errors_second"] += delta * (
                total_errors - rv[key]["total_errors_avg"]
            )
            ...
            rv[key]["total_time_second"] += delta * (
                total_time - rv[key]["total_time_avg"]
            )
    ...
    for key in rv.keys():
        rv[key]["total_errors_var"] = (
            rv[key]["total_errors_second"] / rv[key]["count"][0]
        )
        rv[key]["total_errors_sem"] = np.sqrt(
            rv[key]["total_errors_var"] / rv[key]["count"][0]
        )
        rv[key]["total_time_var"] = rv[key]["total_time_second"] / rv[key]["count"][0]
        rv[key]["total_time_sem"] = np.sqrt(
            rv[key]["total_time_var"] / rv[key]["count"][0]
        )
...
def total_time_metric(buffer_count=0, buffer_value=0):
    return lambda x: x["total_time_avg"] + buffer_value * max(
        buffer_count - x["count"][0], 0
    )
...
def phase_time_metric(phase, buffer_count=0, buffer_value=0):
    return lambda x: sum(
        x["phases_avg"][:phase] + buffer_value * max(buffer_count - x["count"][0], 0)
    )
